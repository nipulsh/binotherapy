-- ============================================
-- Synergy Medical - FINAL Database Schema
-- ============================================
-- This is the complete, corrected schema that matches
-- exactly what the backend expects and supports.
-- 
...
-- Note: Policies will be dropped after tables are created (see RLS POLICIES section)
...
DROP POLICY IF EXISTS "Users can delete own metrics" ON performance_metrics;

-- Productivity logs policies
DROP POLICY IF EXISTS "Users can view own productivity logs" ON productivity_logs;
DROP POLICY IF EXISTS "Users can insert own productivity logs" ON productivity_logs;
DROP POLICY IF EXISTS "Users can update own productivity logs" ON productivity_logs;
DROP POLICY IF EXISTS "Users can delete own productivity logs" ON productivity_logs;

-- Fitness logs policies
DROP POLICY IF EXISTS "Users can view own fitness logs" ON fitness_logs;
...
DROP POLICY IF EXISTS "Users can insert own wellbeing logs" ON wellbeing_logs;
DROP POLICY IF EXISTS "Users can update own wellbeing logs" ON wellbeing_logs;
DROP POLICY IF EXISTS "Users can delete own wellbeing logs" ON wellbeing_logs;

-- Screen time logs policies
DROP POLICY IF EXISTS "Users can view own screentime logs" ON screentime_logs;
DROP POLICY IF EXISTS "Users can insert own screentime logs" ON screentime_logs;
DROP POLICY IF EXISTS "Users can update own screentime logs" ON screentime_logs;
DROP POLICY IF EXISTS "Users can delete own screentime logs" ON screentime_logs;

-- Custom metrics policies
DROP POLICY IF EXISTS "Users can view own custom metrics" ON custom_metrics;
DROP POLICY IF EXISTS "Users can insert own custom metrics" ON custom_metrics;
DROP POLICY IF EXISTS "Users can update own custom metrics" ON custom_metrics;
DROP POLICY IF EXISTS "Users can delete own custom metrics" ON custom_metrics;
...
CREATE POLICY "Users can delete own productivity logs" 
  ON productivity_logs FOR DELETE 
  USING (auth.uid() = user_id);
...
CREATE POLICY "Users can delete own wellbeing logs" 
  ON wellbeing_logs FOR DELETE 
  USING (auth.uid() = user_id);
...
CREATE POLICY "Users can delete own screentime logs" 
  ON screentime_logs FOR DELETE 
  USING (auth.uid() = user_id);
...
BEGIN
  ...
  VALUES (
    ...
    -- OAuth providers use different field names: 'picture' (Google) or 'avatar_url'
    COALESCE(
